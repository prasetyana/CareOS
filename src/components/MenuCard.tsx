...
const MenuCard: React.FC<MenuCardProps> = (props) => {
  const { id, name, description, price, image, badge, rating, soldCount, dietaryTags, isPromo, originalPrice, onAddToCart, onViewDetails } = props;
  ...
  const ActionArea = () => {
    if (!isInteractive || !cartContext) {
      return (
        <div className="w-[32px] h-[32px] bg-accent rounded-full flex items-center justify-center text-white cursor-pointer shadow-sm hover:scale-105 transition-transform">
    ...
    return (
      <button
        className={`w-[28px] h-[28px] bg-accent rounded-full flex items-center justify-center text-white cursor-pointer shadow-sm hover:opacity-90 transition-all duration-200 ${isAnimating ? 'scale-110' : 'hover:scale-105'}`}
    ...
    >
      ...
      <div className="p-3 flex-1 flex flex-col gap-[4px]">
        <div className="flex justify-between items-start gap-2">
          <h3 className="text-[15px] font-medium tracking-tight text-text-primary dark:text-gray-100 leading-tight line-clamp-1">{name}</h3>
        </div>
