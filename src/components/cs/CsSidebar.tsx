...
const CsSidebar: React.FC<CsSidebarProps> = ({ isCollapsed, toggleSidebar }) => {
  const { user, logout } = useAuth();
  const { attentionRequiredCount } = useLiveChat();

  const navItems = [
    { to: "/cs/dasbor", icon: LayoutDashboard, text: "Dasbor" },
    { to: "/cs/live-chat", icon: MessagesSquare, text: "Live Chat" },
    { to: "/cs/pesan-masuk", icon: Inbox, text: "Pesan Masuk" },
    { to: "/cs/kelola-faq", icon: HelpCircle, text: "Kelola FAQ" },
  ];
  
  const navLinkClasses = ({ isActive }: { isActive: boolean }) =>
    `flex items-center px-3 py-2.5 rounded-xl transition-all duration-300 ease-out gap-3 ${
      isActive
  ...
  const collapsedNavLinkClasses = ({ isActive }: { isActive: boolean }) =>
    `relative flex items-center justify-center px-3 py-2.5 rounded-xl transition-all duration-300 ease-out ${
      isActive
        ? 'bg-accent/10 text-accent'
        : 'text-text-muted hover:bg-black/5 dark:hover:bg-white/10 hover:text-text-primary dark:hover:text-gray-200'
    }`;
      ...
      <div>
        <div className={`flex items-center mb-8 transition-all duration-300 ${isCollapsedProp ? 'justify-center' : 'justify-between'}`}>
           <div className={`flex items-center transition-all duration-300 ${isCollapsedProp ? '' : 'justify-between w-full'}`}>
              ...
              >
                <img src={logoDataUri} alt="DineOS Logo" className="h-8 w-8 text-text-primary dark:text-gray-100 flex-shrink-0" />
        ...
        <nav className="space-y-2">
          {navItems.map(item => (
            <div key={item.to} className="relative group">
              <NavLink to={item.to} className={isCollapsedProp ? collapsedNavLinkClasses : navLinkClasses}>
                ...
                {item.text === 'Live Chat' && attentionRequiredCount > 0 && !isCollapsedProp && (
