...
import { MenuItem, addMenuItem, updateMenuItem, fetchCategories, Category } from '../data/mockDB';
import Dropdown from './Dropdown';
import { Save, Image as ImageIcon, Tag, Clock, Flame, DollarSign, AlertCircle, Leaf, Award, Info, Plus, Trash2, Upload } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="flex-grow">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center gap-2 border border-gray-300 dark:border-gray-600 whitespace-nowrap"
                >
                  <Upload size={20} />
                  Upload
                </button>
              </div>
              <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">Gambar pertama akan menjadi gambar utama (thumbnail). Gunakan URL atau upload file lokal.</p>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              {formData.images.map((url, index) => (
                <div key={index} className="relative group rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 aspect-square bg-gray-100 dark:bg-gray-800">
                  <img src={url} alt={`Menu ${index + 1}`} className="w-full h-full object-cover" />
                  ...
                  {index === 0 && (
                    <span className="absolute top-2 left-2 bg-brand-primary text-white text-xs px-2 py-1 rounded-md shadow-sm">
                      Utama
                    </span>
                  )}
                </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label htmlFor="cookTime" className={labelClasses}>Waktu Masak (Estimasi)</label>
            <div className="relative">
          ...
          <div>
            ...
            <div className="relative">
              <Clock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" name="prepTime" id="prepTime" value={formData.prepTime} onChange={handleChange} className={`${inputClasses} pl-10`} placeholder="~5 mnt" />
            </div>
          </div>
          <div>
            <label htmlFor="servingInfo" className={labelClasses}>Info Porsi</label>
            <div className="relative">
              <Info size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" name="servingInfo" id="servingInfo" value={formData.servingInfo} onChange={handleChange} className={`${inputClasses} pl-10`} placeholder="Untuk 2 orang" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="relative">
              <Flame size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" name="mainIngredient" id="mainIngredient" value={formData.mainIngredient} onChange={handleChange} className={`${inputClasses} pl-10`} placeholder="Ayam, Sapi, dll" />
            </div>
          </div>
          <div>
            <label htmlFor="calories" className={labelClasses}>Kalori (kcal)</label>
            <div className="relative">
              <Flame size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
          ...
          <div className="md:col-span-2">
            <label htmlFor="ingredientsString" className={labelClasses}>Komposisi Lengkap (Pisahkan dengan koma)</label>
            <textarea name="ingredientsString" id="ingredientsString" rows={2} value={formData.ingredientsString} onChange={handleChange} className={inputClasses} placeholder="Tepung terigu, telur, gula, susu..."></textarea>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <label className={labelClasses}>Preferensi Diet</label>
            <div className="flex gap-6">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-gray-700 dark:text-gray-300">Halal</span>
              </label>
            </div>
          </div>
          <div>
            <label htmlFor="spicyLevel" className={labelClasses}>Tingkat Pedas</label>
            <select name="spicyLevel" id="spicyLevel" value={formData.spicyLevel} onChange={handleChange} className={inputClasses}>
              <option value="0">Tidak Pedas</option>
              <option value="1">Sedang (Mild)</option>
              <option value="2">Pedas (Medium)</option>
              <option value="3">Sangat Pedas (Hot)</option>
            </select>
          </div>
          <div>
            <label htmlFor="allergensString" className={labelClasses}>Alergen (Pisahkan dengan koma)</label>
            <input type="text" name="allergensString" id="allergensString" value={formData.allergensString} onChange={handleChange} className={inputClasses} placeholder="Kacang, Susu, Gluten..." />
          </div>
          <div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="isPromo" className="text-sm font-medium text-gray-700 dark:text-gray-300 select-none cursor-pointer">
              Aktifkan Harga Promo
            </label>
          </div>

          {formData.isPromo && (
            <div className="animate-slide-in-up">
              <label htmlFor="originalPrice" className={labelClasses}>Harga Asli (Sebelum Diskon)</label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 font-medium">Rp</span>
                <input type="text" name="originalPrice" id="originalPrice" value={formData.originalPrice?.replace('Rp ', '')} onChange={(e) => setFormData(prev => ({ ...prev, originalPrice: `Rp ${e.target.value}` }))} className={`${inputClasses} pl-12`} placeholder="70.000" />
              </div>
              <p className="mt-2 text-xs text-gray-500">Harga di bagian "Informasi Utama" akan menjadi harga diskon.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="pt-4 flex justify-end gap-4">
        <button type="button" onClick={() => window.history.back()} className="px-6 py-3 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          Batal
        </button>
        <button type="submit" disabled={isSubmitting} className="bg-brand-primary text-white px-8 py-3 rounded-lg font-medium hover:bg-brand-primary/90 transition-colors duration-300 disabled:opacity-70 flex items-center gap-2">
          {isSubmitting ? (
            <>
