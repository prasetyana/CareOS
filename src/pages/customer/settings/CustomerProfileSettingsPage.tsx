...
import { User, updateUser } from '../../../data/mockDB';
import { Upload, Loader2, Check, Trash2, X, CheckCircle } from 'lucide-react';
...
const FormField: React.FC<{ label: string; isEditing: boolean; displayContent: React.ReactNode; editContent: React.ReactNode; }> = ({ label, isEditing, displayContent, editContent }) => (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 items-start py-4">
        <dt className="text-sm font-medium text-text-muted col-span-1 pt-2">{label}</dt>
        <dd className="col-span-2 text-text-primary dark:text-gray-200 relative min-h-[40px] flex items-center">
...
const ProfileAvatar: React.FC<ProfileAvatarProps> = ({
    ...
    const AvatarDisplay = () => {
        ...
        return (
            <div className="w-24 h-24 rounded-full bg-accent/10 flex items-center justify-center font-semibold text-5xl text-accent">
    ...
    return (
        <div className="flex flex-col items-center">
                    ...
                    {isEditing && (
                        ...
                        >
                            ...
                            {(avatarPreview || userAvatarUrl) && (
                                <button type="button" onClick={onRemovePreview} className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-semibold border border-black/10 dark:border-white/20 hover:bg-black/5 dark:hover:bg-white/5 text-danger transition-colors">
            ...
            <p className="text-xs text-text-muted mt-2 text-center">JPG, GIF atau PNG. Ukuran maks 800KB.</p>
        </div>
    );
};
...
const ProfileForm: React.FC<ProfileFormProps> = ({ isEditing, formData, errors, isSubmitting, onChange, originalEmail, onVerifyEmail, isVerifyingEmail }) => {
    const getInputBorderClass = (field: keyof typeof errors) =>
        errors[field] ? 'border-danger focus:ring-danger/50' : 'border-black/10 dark:border-white/20 focus:ring-accent';

    const inputClasses = `block w-full shadow-sm py-2 px-3 rounded-lg sm:text-sm transition focus:outline-none focus:ring-1 bg-surface dark:bg-neutral-800 disabled:opacity-50`;
    ...
    return (
        <dl className="divide-y divide-black/10 dark:divide-white/10 -my-4">
            ...
            <FormField
                label="Username"
                isEditing={isEditing}
                displayContent={<p className="text-text-muted">{formData.username || '-'}</p>}
                editContent={
                    <div>
            ...
            <FormField
                label="Alamat Email"
                isEditing={isEditing}
                displayContent={<p className="text-text-muted">{formData.email}</p>}
                editContent={
                    <div>
                        <div className="relative">
                            ...
                            {formData.email !== originalEmail && (
                                <button
                                    ...
                                    className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5 px-3 py-1.5 bg-accent text-white rounded-lg text-xs font-semibold hover:bg-accent/90 transition-colors disabled:opacity-50"
                                >
                                    {isVerifyingEmail ? (
                                        <>
                        ...
                        {formData.email !== originalEmail && (
                            <p className="text-xs text-text-muted mt-1.5">Klik tombol Verifikasi untuk mengonfirmasi email baru</p>
                        )}
                    </div>
                }
            />
            ...
            <FormField
                ...
                editContent={
                    <div>
                        ...
                        {errors.birthdate && <p className="text-xs text-danger mt-1.5">{errors.birthdate}</p>}
                    </div>
                }
            />
        </dl>
...
const CustomerProfileSettingsPage: React.FC = () => {
    ...
    return (
        <div className="space-y-8">
            <div className="bg-surface dark:bg-neutral-800/50 rounded-2xl shadow-card border border-black/10 dark:border-white/10">
                <form onSubmit={handleSubmit}>
                    <div className="p-6 border-b border-black/10 dark:border-white/10 flex justify-between items-center">
                        <div>
                            <h3 className="text-lg font-medium tracking-tight text-text-primary">Informasi Pribadi</h3>
                            <p className="text-sm text-text-muted">Perbarui foto dan detail pribadi Anda di sini.</p>
                        </div>
                        <div className="flex items-center gap-3">
                            {isEditing && (
                                <button type="button" onClick={handleEditToggle} disabled={isSubmitting} className="px-5 py-2 rounded-lg text-sm font-semibold text-text-muted hover:bg-black/5 dark:hover:bg-white/10">
                                    Batal
                                </button>
                            )}
                            {isEditing ? (
                                <button type="submit" disabled={isSubmitting || !isDirty} className="bg-accent text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-opacity-90 transition-colors duration-300 disabled:opacity-50 flex items-center gap-2">
                                    {saveButtonContent()}
                                </button>
                            ) : (
                                <button type="button" onClick={handleEditToggle} className="flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-semibold border border-accent text-accent hover:bg-accent/10 transition-colors">
                                    Edit
                                </button>
                            )}
                        </div>
            ...
            <Modal isOpen={showUnsavedChangesModal} onClose={() => setShowUnsavedChangesModal(false)} title="Perubahan Belum Disimpan">
                <p className="text-text-muted">Anda memiliki perubahan yang belum disimpan. Apakah Anda yakin ingin membatalkannya?</p>
                <div className="flex justify-end gap-4 mt-8">
                    <button onClick={() => setShowUnsavedChangesModal(false)} className="px-5 py-2 rounded-lg text-sm font-semibold text-text-muted hover:bg-black/5 dark:hover:bg-white/10">Lanjut Mengedit</button>
                    <button onClick={handleDiscardChanges} className="bg-danger/10 text-danger px-5 py-2 rounded-lg text-sm font-semibold hover:bg-danger/20">
                        Buang Perubahan
                    </button>
                </div>
            </Modal>
