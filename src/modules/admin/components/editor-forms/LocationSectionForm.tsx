...
import { LocationSectionProps } from '@core/types/homepage';

interface LocationSectionFormProps {
  initialData: LocationSectionProps;
  onSubmit: (data: Partial<LocationSectionProps>) => Promise<void>;
}

const LocationSectionForm: React.FC<LocationSectionFormProps> = ({ initialData, onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label htmlFor="address" className={labelClasses}>Alamat</label>
        <input type="text" name="address" id="address" value={formData.address} onChange={handleChange} className={inputClasses} />
      </div>
       <div>
        <label htmlFor="phone" className={labelClasses}>Telepon</label>
      ...
      <div>
       ...
       <div>
        <label htmlFor="mapSrc" className={labelClasses}>URL Sematan Peta</label>
        <input type="text" name="mapSrc" id="mapSrc" value={formData.mapSrc} onChange={handleChange} className={inputClasses} />
        <p className="text-xs text-brand-secondary dark:text-gray-500 mt-2">
            Salin URL "src" dari kode sematan Google Maps.
        </p>
      </div>
